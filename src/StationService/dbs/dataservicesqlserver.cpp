...
QString DataServiceSqlServer::getSuccessedTradesSql(int vehicleIdentifyType, QString vehPlate, QString cardID, int dataType, int judgeTime) const
{
    ...
    const QString feeField = isEntry ? "0" : "rebatedpay * 100";
    ...
    const QString commonWhere = QString("%1 > DATEADD(SECOND, -%2, GETDATE()) AND IsValid = 1 AND (%3 = CONVERT(varchar(10), GETDATE(), 121) OR %3 = "
                                        "CONVERT(varchar(10), DATEADD(DAY, -1, GETDATE()), 121))")
                                    .arg(timeField)
                                    .arg(judgeTime)
                                    .arg(shiftField);

    sql = QString("SELECT TradeID, %1 AS LaneID, %2 AS TradeTime, %3 AS VehPlate, CardID, %4 AS Fee FROM %5 WHERE %6 AND %7 UNION ALL "
                  "SELECT TradeID, %1 AS LaneID, %2 AS TradeTime, %8 AS VehPlate, CardID, %4 AS Fee FROM %9 WHERE %10 AND %7;")
              .arg(laneField, timeField, etcPlateField, feeField, etcTable, etcWhere, commonWhere, mtcPlateField, mtcTable, mtcWhere);

    return sql;
}

QString DataServiceSqlServer::getShiftCntSql(const QString &shiftDate, int shiftID, int laneID, int flag) const
{
    QString sql;
    if (flag == 1) {
