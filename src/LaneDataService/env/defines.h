...
enum EM_FullBlackStatus {
    ...
    FullBlackUnavailable = -1,     // 启动时无法确定可用全量
    FullBlackCheckFailed = -2,     // 运行中本地、远程清单均无效，可用性保持不变
    FullBlackLocalBatchAhead = -3, // 本地批次高于远程批次，正在按远程批次修复
    FullBlackLocalLoadFailed = -4, // 本地全量加载失败且无法远程恢复，可用性按原值处理
    FullBlackDownloadFailed = -5,  // 远程全量下载失败
    FullBlackVerifyFailed = -6,    // 远程全量完整性或数据库校验失败
    FullBlackPublishFailed = -7    // 远程全量发布失败
};
...
struct ST_EnvSnap
{
    ...
    int fullBlackStatus = FullBlackUnavailable; // 全量状态
