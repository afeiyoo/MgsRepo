...
{
    HttpReply *reply = request(url, operation, body, offset);
    QPair<bool, QByteArray> ans = blockUtilResponse(reply);
    resp = ans.second;
    return ans.first;
}

bool Http::getSync(QByteArray &resp, const QUrl &url)
{
...
bool Http::headSync(QByteArray &resp, const QUrl &url)
{
    HttpReply *reply = head(url);
    QPair<bool, QByteArray> ans = blockUtilResponse(reply);
    resp = ans.second;
    return ans.first;
}

bool Http::postSync(QByteArray &resp, const QUrl &url, const QMap<QString, QString> &params)
{
    HttpReply *reply = post(url, params);
    QPair<bool, QByteArray> ans = blockUtilResponse(reply);
    resp = ans.second;
    return ans.first;
}

bool Http::postSync(QByteArray &resp, const QUrl &url, const QByteArray &body, const QByteArray &contentType)
{
    HttpReply *reply = post(url, body, contentType);
    QPair<bool, QByteArray> ans = blockUtilResponse(reply);
    resp = ans.second;
    return ans.first;
}

bool Http::putSync(QByteArray &resp, const QUrl &url, const QByteArray &body, const QByteArray &contentType)
{
    HttpReply *reply = put(url, body, contentType);
    QPair<bool, QByteArray> ans = blockUtilResponse(reply);
    resp = ans.second;
    return ans.first;
}

bool Http::deleteResourceSync(QByteArray &resp, const QUrl &url)
{
    HttpReply *reply = deleteResource(url);
    QPair<bool, QByteArray> ans = blockUtilResponse(reply);
    resp = ans.second;
    return ans.first;
}

QPair<bool, QByteArray> Http::blockUtilResponse(HttpReply *reply)
{
    QByteArray result;
    ...
    reply->onFinished([&]() {
        if (reply->isSuccessful()) {
            ...
            result = QString("网络错误: %1 (%2)(%3ms)").arg(reply->statusCode()).arg(reply->reasonPhrase()).arg(getReadTimeout()).toUtf8();
        }
        loop.quit();
    });
    ...
    return qMakePair(isSuccessful, result);
}
