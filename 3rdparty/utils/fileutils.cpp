...
bool FileUtils::copyRecursively(const FileName &srcFilePath, const FileName &tgtFilePath, QString *error,
...
bool FileUtils::isFileNewerThan(const FileName &filePath, const QDateTime &timeStamp)
{
    ...
    if (!fileInfo.exists() || fileInfo.lastModified() >= timeStamp)
        return true;
    if (fileInfo.isDir()) {
        const QStringList dirContents = QDir(filePath.toString()).entryList(QDir::Files | QDir::Dirs | QDir::NoDotAndDotDot);
...
QString FileUtils::shortNativePath(const FileName &path)
{
    if (HostOsInfo::isAnyUnixHost()) {
        ...
        if (path.isChildOf(home)) {
            return QLatin1Char('~') + QDir::separator() + QDir::toNativeSeparators(path.relativeChildPath(home).toString());
        }
    }
...
bool FileSaverBase::setResult(bool ok)
{
    if (!ok && !m_hasError) {
        if (!m_file->errorString().isEmpty()) {
            m_errorString = tr("Cannot write file %1: %2").arg(QDir::toNativeSeparators(m_fileName), m_file->errorString());
        } else {
...
TempFileSaver::TempFileSaver(const QString &templ)
{
    ...
    if (!tempFile->open()) {
        m_errorString = tr("Cannot create temporary file in %1: %2")
                            .arg(QDir::toNativeSeparators(QFileInfo(tempFile->fileTemplate()).absolutePath()), tempFile->errorString());
        m_hasError = true;
    }
